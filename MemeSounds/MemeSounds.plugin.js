 ...
 * @source https://github.com/Glinskikh/memesounds2/blob/main/MemeSounds/MemeSounds.plugin.js
 * @updateUrl https://raw.githubusercontent.com/Glinskikh/memesounds2/main/MemeSounds/MemeSounds.plugin.js
 */
	...
	return !global.ZeresPluginLibrary ? class {
		...
		const plugin = (Plugin, Api) => { try {
			...
			const sounds = [
				...
				{re: /нихуя себе/gmi, file: "НИХУЯ СЕБЕ.mp3", duration: 200},
				{re: /да ебись ты/gmi, file: "Да ебись ты.mp3", duration: 500},
				{re: /звучит заманчиво/gmi, file: "ЗВУЧИТ ЗАМАНЧИВО.mp3", duration: 200},
				{re: /cum/gmi, file: "Vergil (1).mp3", duration: 100},
				...
				{re: /oh yes/gmi, file: "mus_ohyes.mp3", duration: 100},
				{re: /za warudo/gmi, file: "hd-stardust-crusaders-za-warudo_1.mp3", duration: 100},
				{re: /wry?y/gmi, file: "dio-wryyy.mp3", duration: 100},
				{re: /pingas/gmi, file: "pingas.mp3", duration: 100},
				{re: /introduction/gmi, file: "introduction.mp3", duration: 100},
				{re: /duel/gmi, file: "the-duel.mp3", duration: 100}
			];
				...
				messageEvent = async ({ channelId, message, optimistic }) => {
					...
					if (!optimistic && lastMessageID != message.id) {
						...
						for (let sound of [...queue.entries()].sort((a, b) => a[0] - b[0])) {
							let audio = new Audio("https://github.com/Glinskikh/memesounds2/raw/main/MemeSounds/Sounds/"+sound[1].file);
